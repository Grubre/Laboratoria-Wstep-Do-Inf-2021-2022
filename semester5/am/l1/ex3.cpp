...
void thread_work(const std::vector<Vec2>& points, const MST& mst, 
    ...
    for(auto i = start; i < end; ++i) {
        ...
        std::shuffle(cycle.begin() + 1, cycle.end() - 1, gen);
        auto [improved_cycle, number_of_moves, improved_cycle_weight] = faster_local_search(points, std::move(cycle));
...
auto main(int argc, char** argv) -> int {
    ...
    const auto work_per_thread = static_cast<std::size_t>(std::ceil(n / static_cast<double>(num_threads)));
    ...
    for (std::size_t i = 0; i < num_threads; ++i) {
        ...
        std::size_t end = std::min((i + 1) * work_per_thread, static_cast<std::size_t>(n));
    ...
    auto average_cycle_weight = cycle_weight_accumulator / n;
    auto average_number_of_moves = number_of_steps_accumulator / n;
