...
struct Result {
    ...
    static Result unexpected(E&& error) {
        return Result(std::move(error));
    }

    static Result expected(T&& value) {
...
namespace std {
    ...
    struct hash<Color> {
        std::size_t operator()(const Color& color) const noexcept {
            return std::hash<uint8_t>()(color.r) ^ std::hash<uint8_t>()(color.g) ^ std::hash<uint8_t>()(color.b);
        }
    };
}
...
inline Result<Image, Err> read_image(const char* filename) {
    ...
    Header header;
    file.read(reinterpret_cast<char*>(&header.id_length), sizeof(header.id_length));
    file.read(reinterpret_cast<char*>(&header.color_map_type), sizeof(header.color_map_type));
    file.read(reinterpret_cast<char*>(&header.image_type), sizeof(header.image_type));
    file.read(reinterpret_cast<char*>(&header.color_map_origin), sizeof(header.color_map_origin));
    file.read(reinterpret_cast<char*>(&header.color_map_length), sizeof(header.color_map_length));
    file.read(reinterpret_cast<char*>(&header.color_map_depth), sizeof(header.color_map_depth));
    ...
    file.read(reinterpret_cast<char*>(&header.pixel_depth), sizeof(header.pixel_depth));
    file.read(reinterpret_cast<char*>(&header.image_descriptor), sizeof(header.image_descriptor));
    ...
    if (header.image_type == 2) {
        for (uint16_t y = 0u; y < header.height; y++) {
            for (uint16_t x = 0u; x < header.width; x++) {
                ...
                file.read(reinterpret_cast<char*>(&colors[index]), sizeof(colors[index]));
            }
        }
    } else {
    ...
    return Result<Image, Err>::expected(Image{header, std::move(id), std::move(colors), std::move(footer)});
}
...
inline Result<bool, Err> save_image(const Image& image, const char* filename) {
    ...
    file.write(reinterpret_cast<const char*>(&image.header.id_length), sizeof(image.header.id_length));
    file.write(reinterpret_cast<const char*>(&image.header.color_map_type), sizeof(image.header.color_map_type));
    file.write(reinterpret_cast<const char*>(&image.header.image_type), sizeof(image.header.image_type));
    file.write(reinterpret_cast<const char*>(&image.header.color_map_origin), sizeof(image.header.color_map_origin));
    file.write(reinterpret_cast<const char*>(&image.header.color_map_length), sizeof(image.header.color_map_length));
    file.write(reinterpret_cast<const char*>(&image.header.color_map_depth), sizeof(image.header.color_map_depth));
    file.write(reinterpret_cast<const char*>(&image.header.x_origin), sizeof(image.header.x_origin));
    file.write(reinterpret_cast<const char*>(&image.header.y_origin), sizeof(image.header.y_origin));
    ...
    file.write(reinterpret_cast<const char*>(&image.header.pixel_depth), sizeof(image.header.pixel_depth));
    file.write(reinterpret_cast<const char*>(&image.header.image_descriptor), sizeof(image.header.image_descriptor));
    ...
    for (uint16_t y = 0u; y < image.header.height; y++) {
        for (uint16_t x = 0u; x < image.header.width; x++) {
            ...
            file.write(reinterpret_cast<const char*>(&image.colors[index]), sizeof(image.colors[index]));
        }
    }
...
inline int compare_images(const Image& image1, const Image& image2) {
    if (image1.header.width != image2.header.width || image1.header.height != image2.header.height) {
        return -1;
    }
...
inline std::ostream& operator<<(std::ostream& os, const Entropy& entropy) {
    os << "(total: " << entropy.total << ", r: " << entropy.r << ", g: " << entropy.g << ", b: " << entropy.b << ")";
    return os;
}
...
inline Entropy entropy(const Image& image) {
    ...
    std::vector<uint64_t> red_counter(channel_size, 0), green_counter(channel_size, 0), blue_counter(channel_size, 0);
    ...
    auto calculate_entropy = [&](const std::vector<uint64_t>& counter) {
        return std::accumulate(counter.begin(), counter.end(), 0.0, [&](double acc, uint64_t count) {
            if (count > 0) {
                double probability = static_cast<double>(count) / (image.header.width * image.header.height);
    ...
    std::vector<uint64_t> total_counter(channel_size * channel_size * channel_size, 0);
    for (const auto& color : image.colors) {
        ++total_counter[color.b * channel_size * channel_size + color.g * channel_size + color.r];
    }
